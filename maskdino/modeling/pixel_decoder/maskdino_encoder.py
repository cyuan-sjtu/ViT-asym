...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.encoder = MSDeformAttnTransformerEncoder(encoder_layer, num_encoder_layers,d_model)
...
class ASYM_CONV(nn.Module):
    def __init__(self, d_model):
        ...
        self.conv_5_5 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(3, 3), stride=1, padding=(1, 1)
            )
         
        self.conv_1_31 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 7), stride=1, padding=(0, 3)
            )
        self.conv_1_21 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 5), stride=1, padding=(0, 2)
            )
        self.conv_1_11 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 3), stride=1, padding=(0, 1)
            )
        self.conv_31_1 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(7, 1), stride=1, padding=(3, 0)
            )
        self.conv_21_1 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(5, 1), stride=1, padding=(2, 0)
            )
        self.conv_11_1 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(3, 1), stride=1, padding=(1, 0)
            )
         
        self.conv_1_1 = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 1), stride=1, padding=(0, 0)
            )
        
        self.conv_1_31n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 7), stride=1, padding=(0, 3)
            )
        self.conv_1_21n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 5), stride=1, padding=(0, 2)
            )
        self.conv_1_11n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 3), stride=1, padding=(0, 1)
            )
        self.conv_31_1n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(7, 1), stride=1, padding=(3, 0)
            )
        self.conv_21_1n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(5, 1), stride=1, padding=(2, 0)
            )
        self.conv_11_1n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(3, 1), stride=1, padding=(1, 0)
            )
         
        self.conv_1_1n = Conv2d(
                self.in_channels, self.in_channels, kernel_size=(1, 1), stride=1, padding=(0, 0)
            )
        
    def forward(self, src):
        src_5_5 = self.conv_5_5(src)
        src_5_5_1 = src_5_5 + \
                    self.conv_31_1(self.conv_1_31(src_5_5)) + \
                    self.conv_21_1(self.conv_1_21(src_5_5)) + \
                    self.conv_11_1(self.conv_1_11(src_5_5))
        src_5_5_1_1 = self.conv_1_1(src_5_5_1) * src_5_5

        src_5_5_2 = src_5_5 + \
                    self.conv_31_1n(src_5_5) + self.conv_1_31n(src_5_5) + \
                    self.conv_21_1n(src_5_5) + self.conv_1_21n(src_5_5) + \
                    self.conv_11_1n(src_5_5) + self.conv_1_11n(src_5_5)
        src_5_5_2_1 = self.conv_1_1n(src_5_5_2) * src_5_5

        return src + src_5_5_1_1 + src_5_5_2_1
...
class MSDeformAttnTransformerEncoder(nn.Module):
    ...
    #         # pdb.set_trace()
    #         accu = 0
    #         for _, spatial_shape in enumerate(spatial_shapes):
                
    #             total_num = torch.prod(spatial_shape)
    #             shape = output.shape
    #             src_unflatten = output[:,accu + 0 : accu + total_num,:].view(tuple(torch.cat((torch.tensor(np.array(shape[0])).cuda().unsqueeze(0),spatial_shape,torch.tensor(np.array(shape[-1])).cuda().unsqueeze(0)),0).tolist()))
    #             accu = accu + total_num
                
    #             output_sub = self.asym(src_unflatten.transpose(1, 3)).transpose(1, 3)
    #             output_flatten.append(output_sub.flatten(1,2))
    #         # pdb.set_trace()
    #         output_asym = torch.cat(output_flatten,1)

    #         output = layer(output_asym, pos, reference_points, spatial_shapes, level_start_index, padding_mask)

    #     return output
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None):
        output = src
        output_vit = torch.Tensor([])
        reference_points = self.get_reference_points(spatial_shapes, valid_ratios, device=src.device)
        for _, layer in enumerate(self.layers):
            ...
            # pdb.set_trace()
            output_asym = torch.cat(output_flatten,1)
            if output_vit.numel() != 0:
                input_vit = output_vit + output_asym
            else:
                input_vit = output_asym
            
            output_vit = layer(input_vit, pos, reference_points, spatial_shapes, level_start_index, padding_mask)
